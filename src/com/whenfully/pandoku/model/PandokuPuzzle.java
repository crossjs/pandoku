...
import com.whenfully.pandoku.solver.DlxPuzzleSolver;
import com.whenfully.pandoku.solver.PuzzleSolver;
import com.whenfully.pandoku.solver.SingleSolutionReporter;
import com.whenfully.pandoku.transfer.StandardAreas;

public class PandokuPuzzle {
	private static final String TAG = PandokuPuzzle.class.getName();
	...
	public PandokuPuzzle(String name, Puzzle puzzle, Level difficulty) {
		if (puzzle == null)
	...
	private boolean restoreFromByteArray(DataInput in) throws IOException {
		Log.d(TAG, "Restoring memento from byte array (Pandoku 1.2.2 and later)");

		short version = in.readShort();

		switch (version) {
			case 5:
				return restoreFromVersion5(in); // Pandoku 1.2.2 and later
			default:
	...
	private boolean restoreFromSerializable(byte[] b) {
		Log.d(TAG, "Restoring memento from Serializable (Pandoku 1.0.0 - 1.2.1)");
	...
	/**
	 * Only maintained for backward compatibility. Pandoku 1.0.0 - 1.2.1 used this class to save the
	 * state of a puzzle. Pandoku 1.2.2 and later directly write to a byte array instead.
	 */
	private static final class PuzzleMemento implements Externalizable {
		...
		public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException {
			...
			switch (version) {
				case 3:
					readVersion3(in); // Pandoku 1.0.0 - 1.0.2
					break;
				case 4:
					readVersion4(in); // Pandoku 1.1.0 - 1.2.1
					break;
				default:
